...
func (O mt_seq_no) Get(p *Package) (int, int) {
...
func (O mt_seq_no) Remove(p *Package) {
	for e := p.Front(); e != nil; e = e.Next() {
		if e.MessageType() == MT_SEQ_NO {
			p.Remove(e)
			break
...
func (O mt_seq_no) Set(p *Package, seqno, seqmax int) {
	O.Remove(p)
	f := NewFrameV(MT_SEQ_NO, &struct_seq_no{seqno, seqmax}, O)
	p.PushFront(f)
}

func (O mt_seq_no) IsLastSeq(p *Package) bool {
	n, m := O.Get(p)
	return O.IsLast(n, m)
}
