...
func (O mt_payload) Get(p *protpack.Package) []byte {
...
func (O mt_payload) Remove(p *protpack.Package) {
	for e := p.Front(); e != nil; e = e.Next() {
		if e.MessageType() == MT_PAYLOAD {
			p.Remove(e)
			break
...
func (O mt_payload) Set(p *protpack.Package, val interface{}) {
	for e := p.Front(); e != nil; e = e.Next() {
		if e.MessageType() == MT_PAYLOAD {
			p.Remove(e)
			break
	...
	f := protpack.NewFrameV(MT_MESSAGE_ID, val, O)
	p.PushFront(f)
}
