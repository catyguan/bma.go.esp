...
func (this *PChannel) SetMessageListner(rec esnp.MessageListener) {
	this.lis = rec
	ch := this.channel()
	if ch != nil {
...
func (this *PChannel) SendMessage(ev *esnp.Message) error {
	l := len(this.items)
	for i := 0; i < l; i++ {
		ch := this.channel()
		if ch == nil {
...
func (this *PChannel) myMessageListener(msg *esnp.Message) error {
	if this.lis == nil {
