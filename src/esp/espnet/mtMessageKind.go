...
func (O mt_message_kind) Get(p *protpack.Package) MessageKind {
...
func (O mt_message_kind) Remove(p *protpack.Package) {
	for e := p.Front(); e != nil; e = e.Next() {
		if e.MessageType() == MT_MESSAGE_KIND {
			p.Remove(e)
			break
...
func (O mt_message_kind) Set(p *protpack.Package, val MessageKind) {
	for e := p.Front(); e != nil; e = e.Next() {
		if e.MessageType() == MT_MESSAGE_KIND {
			p.Remove(e)
			break
	...
	f := protpack.NewFrameV(MT_MESSAGE_KIND, val, O)
	p.PushFront(f)
}
