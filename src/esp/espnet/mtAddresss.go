...
func (O addrCoder) is(e *protpack.Frame, ann int) (*struct_address_value, error) {
...
func (O addrCoder) Set(p *protpack.Package, ann int, value string) {
	...
	f := protpack.NewFrameV(O.MT(), &struct_address_value{ann, value}, O)
	p.PushFront(f)
}

func (O addrCoder) Get(p *protpack.Package, ann int) (string, error) {
...
func (O addrCoder) Remove(p *protpack.Package, ann int) {
	p.RemoveFrame(func(e *protpack.Frame) (bool, bool) {
		if mv, _ := O.is(e, ann); mv != nil {
			return true, false
		}
...
func (O addrCoder) Clear(p *protpack.Package) {
	p.RemoveFrame(func(e *protpack.Frame) (bool, bool) {
		if e.MessageType() == O.MT() {
			return true, false
		}
...
func (O addrCoder) List(p *protpack.Package) []int {
	r := make([]int, 0)
	mt := O.MT()
	for e := p.Front(); e != nil; e = e.Next() {
		if e.MessageType() != mt {
