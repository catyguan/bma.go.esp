...
func (this *cacheConfig) GetUProperties() []*uprop.UProperty {
	...
	r = append(r, uprop.NewUProperty("iholder", this.InvalidHolder, true, "create invalid holder", func(v string) error {
		this.InvalidHolder = valutil.ToBool(v, this.InvalidHolder)
		return nil
	}))
	r = append(r, uprop.NewUProperty("update", this.UpdateSeconds, true, "updater run duration(sec.), <=0 mean stop", func(v string) error {
		this.UpdateSeconds = valutil.ToInt32(v, 0)
		return nil
	}))
	r = append(r, uprop.NewUProperty("step", this.UpdateStep, true, "updater scan step, default 10", func(v string) error {
