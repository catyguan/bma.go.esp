...
func NewTFramedTransportFactory(factory TTransportFactory, maxframe func() uint64) TTransportFactory {
	return &tFramedTransportFactory{factory: factory, maxFrame: maxframe}
}

func (p *tFramedTransportFactory) GetTransport(base TTransport) TTransport {
	return NewTFramedTransport(p.factory.GetTransport(base), p.maxFrame)
}

func NewTFramedTransport(transport TTransport, maxframe func() uint64) *TFramedTransport {
	writeBuf := make([]byte, 0, 1024)
	readBuf := make([]byte, 0, 1024)
	return &TFramedTransport{transport: transport, writeBuffer: bytes.NewBuffer(writeBuf), readBuffer: bytes.NewBuffer(readBuf), maxFrame: maxframe}
}
...
func (p *TFramedTransport) readFrame() (int, error) {
	...
	if mf > 0 && uint64(size) > mf {
		return 0, NewTTransportException(UNKNOWN_TRANSPORT_EXCEPTION, "Read a max frame size ("+string(size)+")")
	}
	buf2 := make([]byte, size)
